namespace Refundation_App_Services.Extensions
{
    public static class Extensions
    {
        public static string ToHtmlTable<T>(this List<T> objectList)
        {
            ...
            return objectList == null || !objectList.Any()
                ? ret
                : "<table>" +
                  objectList.First().GetType().GetProperties().Select(p => p.Name).ToList().ToColumnHeaders() +
                  objectList.Aggregate(ret, (current, t) => current + t.ToHtmlTableRow()) +
                  "</table>";
        }
